...
class MixedPeopleDataset(Dataset):
    ...
    混合不同人数的数据集
    """
    def __init__(self, data_dir, t_his=16, t_pred=14, split='train'):
        """
        Args:
            data_dir: 数据目录
            t_his: 输入帧数
        ...
        # 根据split选择数据文件
        if self.split == 'train':
            # 训练集：只使用train_3_120_mocap.npy
            data_files = {
                'train_3_120_mocap.npy': 3
            }
        else:  # test
            # 测试集：使用mupots、test数据以及mix数据集
            data_files = {
                #'mupots_120_3persons.npy': 3,
                #'test_3_120_mocap.npy': 3,
                #'mix1_6persons.npy': 6,
                'mix2_10persons.npy': 10
            }
        ...
        for filename, expected_people in data_files.items():
            ...
            if os.path.exists(filepath):
                try:
                    ...
                    # 处理数据格式 - 参考dataset_mocap.py
                    # 120帧的mocap数据需要间隔采样
                    if filename in ['mupots_120_3persons.npy', 'train_3_120_mocap.npy', 'test_3_120_mocap.npy']:
                        # mocap数据处理：使用间隔采样 [N, P, T(120), JK] -> [N, P, T(60), JK]
                        if len(data.shape) == 4:  # [N, P, T, JK]
                            # 使用::2进行下采样，从120帧采样到60帧
                            data = data[:, :, ::2, :]  # [N, P, 60, JK]
                            # 然后转换为5维格式 [N, P, T, J, 3]
                            data = data.reshape(data.shape[0], data.shape[1], data.shape[2], -1, 3)
                        else:
                            print(f"Unexpected mocap data shape for {filename}: {data.shape}")
                            continue
                    else:
                        # mix数据集和其他数据文件的处理
                        if len(data.shape) == 4:  # [N, P, T, JK]
                    ...
                    if data.shape[2] >= total_frames:
                        # 直接使用所有序列，不需要再分割（因为训练和测试用的是不同文件）
                        num_sequences = data.shape[0]
                        sequence_indices = range(0, num_sequences)
