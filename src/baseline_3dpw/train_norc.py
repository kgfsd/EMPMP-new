...
# 定义 default_serializer 函数，可以放在外面
...
if __name__ == '__main__':
    ...
    try:
        ...
        # train_step 函数定义
        def train_step(h36m_motion_input, h36m_motion_target, padding_mask, model, optimizer, nb_iter, total_iter,
                       max_lr, min_lr):
            if args.random_rotate:
                h36m_motion_input, h36m_motion_target = getRandomRotatePoseTransform(h36m_motion_input,
                                                                                     h36m_motion_target)
            if args.paixu:  # DCT之前
                h36m_motion_input, h36m_motion_target, sorted_indices = paixu_person(h36m_motion_input,
                                                                                     h36m_motion_target)
            if args.permute_p:
