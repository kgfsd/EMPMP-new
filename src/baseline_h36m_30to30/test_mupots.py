import torch
import numpy as np
from src.models_dual_inter.utils import predict# Very important, when switching models, change models_inter to other folders
from src.models_dual_inter.utils import visuaulize
...
def regress_pred(model,motion_input,config):# Recursive prediction, suitable for cases where output frames exceed input frames, but recursion is not needed here
...
def mpjpe_test_regress(config, model, eval_generator,dataset="mocap"):    
    ...
    for (h36m_motion_input, h36m_motion_target) in eval_generator:
        ...
        motion_pred = regress_pred(model,h36m_motion_input,config)# Prediction result
...
def mpjpe_vim_test(config, model, eval_generator,is_mocap,select_vim_frames=[1, 3, 7, 9, 13],select_mpjpe_frames=[10,20,30]):    
    ...
    def cal_vim(motion_pred,h36m_motion_target,vim_avg):    
        ...
        for person in range(p):
            # if person==1:
            #     print("Skip the second person")
            #     continue
            JK=c
            K=3
            J=c//K
            i=0
            for k in range(len(h36m_motion_target)):# k is sample index
    ...
    def cal_mpjpe(motion_pred,h36m_motion_target,is_mocap,select_frames=[10,20,30]):    
        ...
        else: # mupots dataset or 3dpw dataset
...
def vis_all(config, model, eval_generator,dataset="mocap"):    
    device=config.device
    # dct_m=config.dct_m
    # idct_m=config.idct_m
    n_joint=config.n_joint
    ...
    for (h36m_motion_input, h36m_motion_target) in eval_generator:
        ...
        motion_pred = motion_pred.reshape(b,p,n,n_joint,3).cpu().detach()
        h36m_motion_target=h36m_motion_target.reshape(b,p,n,n_joint,3).cpu().detach()#b,p,t,j,3
        h36m_motion_input=h36m_motion_input.reshape(b,p,n,n_joint,3).cpu().detach()
        prediction=torch.cat([h36m_motion_input,motion_pred],dim=2).numpy()
        gt=torch.cat([h36m_motion_input,h36m_motion_target],dim=2).numpy()
        visuaulize(prediction,f"pred_sample{sample}",config.vis_dir,input_len=30)# Visualization
        visuaulize(gt,f"gt_sample{sample}",config.vis_dir,input_len=30)# Visualization
