import torch
import numpy as np
from src.models_dual_inter_traj_pips.utils import predict#很重要，切换模型时将models_inter修改为其他文件夹
from src.models_dual_inter_traj_pips.utils import visuaulize,paixu_person
...
def vis_all(config, model, eval_generator,dataset="mocap"):    
    ...
    for (h36m_motion_input, h36m_motion_target) in eval_generator:
        ...
        visuaulize(prediction,f"pred_sample{sample}",config.vis_dir,input_len=30)#可视化
        visuaulize(gt,f"gt_sample{sample}",config.vis_dir,input_len=30)#可视化
