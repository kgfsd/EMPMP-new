...
class StylizationBlock(nn.Module):
    ...
    def forward(self, x, x_global,distances):
        ...
        if P < self.max_p:
            ...
            # Pad distances if using distance
        ...
        if post_x:# Update x first, then use updated x to update x_global
            ...
            x_padded = self.out_layers(x_padded)
            if self.use_distance:
                distances_padded=self.dis_linear(distances_padded)#B,max_p,1,D -> actually expects B,T,P,D based on comment
                distances1=distances_padded.permute(0,2,3,1)#B,P,D,T
