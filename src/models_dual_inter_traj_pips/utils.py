...
from src.models_dual_inter_traj_pips.model import compute_distances_hierarchical,sort_tensor_by_distance,inverse_sort_tensor
...
def paixu_person(h36m_motion_input,h36m_motion_target):
    b,p,t_in,jk=h36m_motion_input.shape
    b,p,t_out,jk=h36m_motion_target.shape
    k=3
    j=jk//k
    h36m_motion_input=h36m_motion_input.reshape(b,p,t_in,j,k)
    h36m_motion_target=h36m_motion_target.reshape(b,p,t_out,j,k)
    
    first_distance=compute_distances_hierarchical(h36m_motion_input)
    h36m_motion_input,sorted_indices_input=sort_tensor_by_distance(first_distance,h36m_motion_input)
    h36m_motion_target,sorted_indices_target=sort_tensor_by_distance(first_distance,h36m_motion_target)
    assert torch.equal(sorted_indices_input,sorted_indices_target)
    sorted_indices=sorted_indices_input
    
    h36m_motion_input=h36m_motion_input.reshape(b,p,t_in,jk)
    h36m_motion_target=h36m_motion_target.reshape(b,p,t_out,jk)
    return h36m_motion_input,h36m_motion_target,sorted_indices
